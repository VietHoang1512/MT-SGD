...
def get_dataset(params, configs):
    ...
    if "mnist" in params["dataset"]:
        train_dst = MNIST(
            root=configs["mnist"]["path"], train=True, download=True, transform=global_transformer(), multi=True
        )
        ...
        val_dst = MNIST(
            root=configs["mnist"]["path"], train=False, download=True, transform=global_transformer(), multi=True
        )
