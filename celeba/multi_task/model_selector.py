...
def get_model(params, num_nets=1):
    data = params["dataset"]
    if "mnist" in data:
        model = {}
    ...
    if "celeba" in data:
        ...
        if num_nets == 1:
            ...
            for t in params["tasks"]:
                ...
                model[t].cuda()
            return model
        model["rep"] = [ResNet(BasicBlock, [2, 2, 2, 2]).cuda() for _ in range(num_nets)]
        for t in params["tasks"]:
            model[t] = [FaceAttributeDecoder().cuda() for _ in range(num_nets)]
        return model
