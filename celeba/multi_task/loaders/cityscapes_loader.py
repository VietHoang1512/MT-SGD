...
class CITYSCAPES(data.Dataset):
    ...
    def __init__(self, root, split=["train"], is_transform=False, img_size=(512, 1024), augmentations=None):
        """__init__
    ...
    def __getitem__(self, index):
        ...
        depth_path = os.path.join(
            self.depth_base, img_path.split(os.sep)[-2], os.path.basename(img_path)[:-15] + "disparity.png"
        )
    ...
    def transform(self, img, lbl, ins_y, ins_x, depth):
        ...
        lbl = m.imresize(
            lbl, (int(self.img_size[0] / 8), int(self.img_size[1] / 8)), "nearest", mode="F"
    ...
    def decode_segmap(self, temp):
        ...
        for n in range(0, self.n_classes):
            r[temp == n] = self.label_colours[n][0]
            g[temp == n] = self.label_colours[n][1]
            b[temp == n] = self.label_colours[n][2]
